...
public:
	...
	void setProjection(ID3D11DeviceContext *a_pDeviceContext, const XMFLOAT3 &a_preSetScale = XMFLOAT3(1, 1, 1), const XMFLOAT3 &a_position = XMFLOAT3(0, 0, 0), const Transform& a_pCustom3D = Transform::initialValue(), const XMMATRIX &a_globalTransform = XMMatrixIdentity(), SkeletalAnimation &a_skeletalAnimation = SkeletalAnimation(), float a_elapsedTime = 1 / 60.0f/*UNIT.23*/);
	void render(ID3D11DeviceContext *a_pDeviceContext, bool a_doFill, const Mesh &a_mesh);

	void drawMesh(ID3D11DeviceContext *a_pDeviceContext, const XMFLOAT3 &a_preSetScale = XMFLOAT3(1, 1, 1), const XMFLOAT3 &a_position = XMFLOAT3(0, 0, 0), const Transform& a_pCustom3D = Transform::initialValue(), float a_elapsedTime = 1 / 60.0f/*UNIT.23*/);
};
